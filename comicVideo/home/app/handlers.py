...
def check_super_admin(request):
    if request.__user__ is None or not request.__user__.admin == 99:
...
def index_mzt(*, page='1'):
    page_index = get_page_index(page)
    num = yield from MPicture.findNumber('count(id)')
    page = Page(num, page_index=page_index, page_size=12, page_url='/mZiTu.html?page=')
    pictures = yield from MPicture.findAll(where='1=1', orderBy='created_at desc', limit=(page.offset, page.limit))
    # for p in pictures:
    #     p.pic_url = ''
    return {
...
@get('/mZiTu/getPicSource')
def picture_resource(*, pic_id=None, url=''):
    if url == '' or pic_id is None:
        return {'status': 'fail', 'img': '/static/ui/img/404.png'}
    if pic_id in data_local:
        return {'status': 'ok', 'img': data_local[pic_id]}
    mz = MZiTu()
    try:
        base64_data = mz.request_referrer(url, 'http://www.mzitu.com/2018/1')
    except BaseException:
        logging.info('[%s]获取base64 源码图片链接异常[%s] ' % (pic_id, url))
        return {'status': 'fail', 'img': '/static/ui/img/404.png', 'msg': '网络连接超时'}
    if base64_data is None:
        return {'status': 'fail', 'img': '/static/ui/img/404.png'}
    else:
        img = 'data:image/png;base64,' + str(base64_data, encoding="utf-8")
        data_local[pic_id] = img
    return {'status': 'ok', 'img': img}


@get('/mZiTu/picGroup')
def get_pic_group(*, group_id=None):
    if group_id is None:
        return {'status': 'fail', 'msg': '请求操作异常'}
    pic = yield from MPictureInfo.findAll(where='group_id=?', args=[group_id], orderBy='img_order asc')
    return {'status': 'ok', 'list': pic}


# 后台操作mZiTu 获取索引
@post('/api/mIndex')
def mzt_index(request, *, base_url='http://www.mzitu.com/all', thread_count=1, trans_id=next_id()):
    check = check_super_admin(request)
    if check:
        return check

    num = yield from MIndex.findNumber('count(id)')
    if num > 0:
        return {'status': 'fail', 'err': '已经全量更新获取了此部分数据!'}

    if thread_count > 1:
        return {'status': 'fail', 'err': '此部分内容暂时不支持并发操作，请按照要求操作!'}
    # 线程监控
    monitor_thread = []
    args = {
        'base_url': base_url,
    ...
    mzt = threading.Thread(target=process_thread_mzt, args=(args,), name='妹子图索引地址解析服务')
    monitor_thread.append(mzt)
    # 启动日志监控
    monitor_args = {
        'monitor_thread': monitor_thread,
        'message_type': '妹子图加载全部地址索引',
        'trans_id': trans_id,
        'content': "初始化数据服务：详细【%s部分内容】" % base_url,
        'message_order': 1,
        'status': '预备等待执行...'
    }
    monitor = threading.Thread(target=process_thread_monitor, args=(monitor_args,), name='线程监控服务')
    monitor.start()
    return {'status': 'ok', 'msg': '你的操作已经已经提交，奴家正在拼命...'}


# 套图组
@post('/api/pic/group')
def mzt_pic_group(request, *, thread_count=1, trans_id=next_id()):
    check = check_super_admin(request)
    if check:
        return check
    ...
    all_index = yield from MIndex.findAll(orderBy='url_order desc')
    if all_index is None or len(all_index) == 0:
        return {'status': 'fail', 'err': '无记录信息，请联系超级管理员!'}
    # 计算线程数量
    len_all = len(all_index)
    thread_count = len_all // 20
    if thread_count > 20:
        thread_count = 20
    if thread_count == 0:
        thread_count = 1
    # 测试
    # thread_count = 1
    size = len_all // thread_count
    if len_all % thread_count != 0:
        size = size + 1
    # 线程监控
    monitor_thread = []
    for i in range(0, thread_count):
        ...
        if start > len_all:
            break
        args = {
            'index_url': all_index[start:end],
            'method': 'pic_group'
        }
        mt = threading.Thread(target=process_thread_mzt, args=(args,), name='妹子图套图组解析服务:' + str(i + 1) + '部分')
        monitor_thread.append(mt)

    # 启动日志监控
    monitor_args = {
        'monitor_thread': monitor_thread,
        'message_type': '妹子图加载各个索引套图',
        'trans_id': trans_id,
        'content': "初始化数据服务：详细【%s部分内容】" % len_all,
        'message_order': 1,
        'status': '预备等待执行...'
    }
    monitor = threading.Thread(target=process_thread_monitor, args=(monitor_args,), name='线程监控服务')
